...
import os
import random
import shutil  # <-- Ajouté pour permettre la copie de fichiers
...
def show_project_menu(widget, width, height):
    def add_image_to_folder(folder_name):
        ...
        if file_path:
            ...
            shutil.copy(file_path, destination)  # <-- Copie l'image au lieu de la déplacer
    ...
    Button(screen, text="Ajouter un visage....", command=lambda: add_image_to_folder("Face")).place(x=600, y=100)
    Button(screen, text="Ajouter des cheveux..", command=lambda: add_image_to_folder("Hair")).place(x=600, y=150)
    Button(screen, text="Ajouter un corps.....", command=lambda: add_image_to_folder("Body")).place(x=600, y=200)
